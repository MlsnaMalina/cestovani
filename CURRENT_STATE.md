...
| Výlety od ubytování | 6 SI + 9 HR (přidáno 11. 9. 2026: Šibenik, Skradin, NP Krka; vrácena i dřív vyřazená Lublaň), sluníčka, přiblížení okolí, fotky, podrobnosti. Limit zvednut z 60 na 100 minut na výslovné zadání uživatelky; cokoli nad 60 minut tam/zpět má zřetelnou výstrahu (karta i detail). | Hotové. Nepřidávají se samy do přejezdů/rozpočtu. |
| Zdravotní/bezpečnostní upozornění | Nový sdílený katalog `data/health-advisories.json` (klíšťová encefalitida u lesních zastávek, mořští ježci/medúzy u koupacích výletů, poskok v Paklenici). Výstražný trojúhelník na kartě i mapové značce, po kliknutí detail s doporučením a zdroji v `HealthAdvisories.tsx`. Prověřeno i zamítnuto: západonilská horečka — 0 případů v Zadarské župě 2026, neplatí pro tuto trasu. | Hotové 11. 9. 2026. Ověřeno runtime přes kartu, detail a data (karty, upozornění, ceny); mapová značka s trojúhelníkem zůstává k ověření na skutečném zařízení — mapa se v tomto testovacím prostředí (skrytý panel) nevykreslila. |

...
export default function MapView(props:Props){
 ...
 useEffect(()=>{
  ...
  const render=()=>{tripMarkers.forEach(m=>m.remove());const groups:{points:Excursion[];x:number;y:number}[]=[];
   ...
   tripMarkers=groups.map(group=>{const p=group.points[0],el=document.createElement('button');el.type='button';el.className='excursion-marker';el.textContent='☀';el.title=group.points.length>1?group.points.length+' výletů v okolí':p.name;el.setAttribute('aria-label',group.points.length>1?'Skupina výletů: '+group.points.length:'Výlet: '+p.name);if(group.points.length>1){const count=document.createElement('b');count.textContent=String(group.points.length);el.append(count);}el.addEventListener('click',event=>{event.stopPropagation();if(group.points.length>1)current.current.onExcursionGroup(group.points.map(p=>p.id));else current.current.onExcursion(p.id);});return new maplibregl.Marker({element:el,offset:[16,0]}).setLngLat(p.drive.coordinates).addTo(map);});

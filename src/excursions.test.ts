...
import {excursions,overHour} from './excursionData';
import photos from '../data/photos.json';
import raw from '../public/data/trip.json';
import {defaults,fuel} from './planning';
import type {Dataset} from './types';
describe('Výlety do 100 minut od ubytování',()=>{
 it('nabízí obě ubytování a jen silniční dojezdy do 100 minut v obou směrech',()=>{
  expect(excursions.filter(p=>p.base==='si')).toHaveLength(6);expect(excursions.filter(p=>p.base==='hr')).toHaveLength(9);
  expect(new Set(excursions.map(p=>p.id)).size).toBe(excursions.length);
  expect(excursions.some(p=>p.id==='trip-ljubljana')).toBe(true);
  for(const p of excursions){expect(p.drive.drivingMinutes).toBeGreaterThan(0);expect(p.drive.drivingMinutes).toBeLessThanOrEqual(100);expect(p.drive.returnMinutes).toBeGreaterThan(0);expect(p.drive.returnMinutes).toBeLessThanOrEqual(100);}
 });
 it('označí výstrahou jen výlety nad hodinu tam nebo zpět',()=>{
  const over=excursions.filter(p=>overHour(p.drive)).map(p=>p.id).sort();
  expect(over).toEqual(['trip-krka','trip-ljubljana','trip-sibenik'].sort());
  expect(overHour(excursions.find(p=>p.id==='trip-skradin')!.drive)).toBe(false);
 });
 ...
 it('odděluje placenou dálniční alternativu a počítá benzín z obou různých směrů',()=>{
  expect(excursions.filter(p=>p.drive.paidAlternative).map(p=>p.id).sort()).toEqual(['trip-krka','trip-ljubljana','trip-paklenica','trip-sibenik','trip-skradin'].sort());
  const p=excursions.find(p=>p.id==='trip-biograd')!,settings=defaults(raw as unknown as Dataset);

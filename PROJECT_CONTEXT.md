...
- Vercel: konfigurace připravena, propojení a veřejná produkční URL nejsou doložené. Push na GitHub není potvrzením nasazení.
...
Commit a push hotové ověřené práce na `main` jsou předem autorizované; autor `k.schmiedtova@seznam.cz`. Žádné tajné klíče v kódu/Gitu, `.env` ignorovat. Technické oprávnění sandboxu k zápisu do `.git` nebo síti může být nutné i přes uživatelskou autorizaci. Další agenty nespouštět bez výslovného požadavku nebo použitelných instrukcí.
...
- Není doložená veřejná produkční URL ani Vercel projekt. Není synchronizace mezi zařízeními, export/import plánu ani garantovaný offline provoz.
